...
def is_main_process():
    """检查当前进程是否为主进程（分布式训练中只有 rank 0 执行日志/保存）"""
...
def get_lr(current_step, total_steps, lr):
    """余弦退火学习率：从 lr 衰减到 0.1*lr"""
    return lr * (0.1 + 0.45 * (1 + math.cos(math.pi * current_step / total_steps)))


def init_distributed_mode():
    """初始化分布式训练环境，返回当前进程的 GPU 编号"""
        ...
        for key, value in kwargs.items():
            if value is not None:
                if hasattr(value, "state_dict"):
                    resume_data[key] = value.module.state_dict() if isinstance(value, DistributedDataParallel) else value.state_dict()
                else:
...
def init_model(
    ...
    """初始化模型和 tokenizer，可选加载已有权重"""
    from transformers import AutoTokenizer
    from model.model import HelloModelForCausalLM
    ...
    model = HelloModelForCausalLM(lm_config)
